...
- Auction-house transaction mechanics remain player-capable, but the old `/player/auction-house` visual route was removed with the UI reset.
...
- Player combat masking mechanics remain available through core logic; the old `/player/combat` visual route was removed with the UI reset.
- Hidden opponents should still stay masked as `Opponent N` in any rebuilt player combat UI unless the viewing character owns an `Assess Entity` knowledge card for that target.
- Supabase-backed live session services can use explicit config or `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` process env values; local-only behavior remains available when config is absent.
- The old live session routes were removed with the previous UI and must be rebuilt on top of `OnlineSessionService` and `realtimeSessionRepository` in the next UI.
...
- DM authoring data now includes mobs, mob groups, and portal templates; the old dedicated visual routes were removed with the UI reset.
...
- The preferred Codex authoring data workflow remains portal-first through `portal_bundle` request packets, but the old `/dm/portals` visual route was removed with the UI reset.
...
- DM knowledge data supports non-character/item subject authoring; the old Knowledge Hub route was removed with the UI reset.
...
- New player/DM UI design is not implemented yet; it is tracked as `NEW-UI-DESIGN-01`.
